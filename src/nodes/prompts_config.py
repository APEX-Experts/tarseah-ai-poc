...
SECTIONS_CONFIG: Dict[str, SectionConfig] = {
    ...
    "company_profile": {
        "system_prompt": (
            ...
            "# CORE OBJECTIVE\n"
            "Write an official corporate company profile section based strictly and only on the company records and assets found within the provided context database. "
    ...
    "past_projects": {
        "system_prompt": (
            ...
            "# CORE OBJECTIVE\n"
            "Write the 'Past Projects' or 'Track Record' (سوابق الأعمال) section. Extract, structure, and present historical projects, platform products, or execution case studies executed by our company, using strictly the verified records found within the provided company documents.\n\n"
            "# DYNAMIC RELEVANCE & ALIGNMENT ANALYSIS:\n"
            "- **Show Sector Relevance**: For each genuine past project extracted, analyze its scope and dynamically describe how it maps to or supports the execution of the current tender requirements (found under RFP Documents).\n"
   ...
   # ── 8. Team Structure ────────────────────────────────────────────────
    "team": {
        "system_prompt": (
            ...
            "Your objective is to extract the main corporate departments and delivery functions from the provided Company Profile and dynamically map them to fulfill the RFP's operational requirements.\n\n"
  ...
  # ── 11. Financial Proposal (Pricing Section) ───────────────────────
    "pricing": {
        "system_prompt": (
            ...
            "# CORE OBJECTIVE\n"
            "Write the complete 'Financial Proposal' (العرض المالي والتسعير). Your goal is to analyze the project scope, deliverables, and the Bill of Quantities (BOQ) from the RFP, and dynamically calculate and inject fully-formed, highly realistic numeric price estimates. "
            "Do NOT leave blank cells, underscores, or empty placeholders. Every cost item must have a calculated numeric value.\n\n"
            
            "# SMART SECTOR-BASED ESTIMATION LOGIC:\n"
            "- **Context-Driven Estimation**: Analyze the total volume of deliverables in the RFP (e.g., number of training courses, operational days, human resources, logistics, international partnerships, or physical materials). Estimate a mathematically sound, market-compatible price based on the scale of the target project.\n"
            "- **Data-Driven Pivot**: Check for any explicit rate cards or financial benchmarks in the provided context files. If missing, automatically deploy a high-level corporate lump-sum and itemized costing model matching the actual scale of the RFP.\n\n"
            
            "# STRICT MATHEMATICAL INTEGRITY & WEIGHTS:\n"
            "- **100% Mathematical Accuracy**: Every line item (Unit Price * Quantity) must sum up perfectly to the Subtotals, and all Subtotals must perfectly equal the Grand Total Cost.\n"
            "- **Payment Milestones Sync**: Create a milestone payment schedule table aligned relatively with the project phases (e.g., Month 1, Months 2-4). The payment percentages injected for milestones MUST mathematically sum up to exactly 100%.\n"
            "- **Guarantees and Taxes**: Calculate the exact required initial guarantee (e.g., 1% of your calculated grand total) and final guarantee (e.g., 5% of your calculated grand total) if mandated by the RFP. Clearly state that all injected figures are inclusive of the official Saudi Value Added Tax (VAT 15%) and all local banking fees.\n\n"
            ...
            "2. **Detailed Bill of Quantities (BOQ) Table**: Generate a complete Markdown table displaying the itemized costing. Columns exactly: (رقم البند, وصف البند, الوحدة, الكمية, السعر الإفرادي (SAR), إجمالي التكلفة (SAR)).\n"
            "3. **Milestone Schedule Table**: Columns exactly: (معلم الدفع / المرحلة, نسبة الدفعة (%), المبلغ المستحق (SAR), موعد الاستحقاق النسبي, ملاحظات الصرف).\n"
